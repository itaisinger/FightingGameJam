...
arr_state_functions[STATES.walk] = function(){
	...
	if(_press != press_prev and _press != 0 or reached_frame(image_number-1)) create_vfx(x,y,vfx_run_brown,dir*RUN_VFX_S,RUN_VFX_S,0);
...
function collision(){
	...
	//hor
		if(place_meeting(x+xadd,y,obj_wall) and state == STATES.air_stun){ xadd *= -1.15; dir *= -1}
