...
if(!is_parry)
{
	...
	for(var i=0; i < _num and !stop; i++){
		...
		if(_col.is_p1 != parent.is_p1 and !array_contains(arr_hits,_col)){
			...
			//hit
			_col.hit(damage,knockback_x*dir,knockback_y,stun_duration,hitpause,is_launcher,false);
			...
			//strong sfx
			_gain = max(0,map_value(damage,6,15, 0,1));
			play_sfx(sfx_strong,0,0,[0.8,1.2],_gain)
