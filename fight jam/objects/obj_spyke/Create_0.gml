...
hitbox_data[STATES.heavy]		= new HitboxData(hitbox_spyke_heavy,8,70,15,2,10,0,1,false,,,,,sfx_lightning_hit);
hitbox_data[STATES.special]		= new HitboxData(hitbox_spyke_special,9,60,10,7,5,0,1,false,,,,,sfx_lightning_hit);
hitbox_data[STATES.air_light]	= new HitboxData(hitbox_spyke_air_light,5,60,5,3,7,0,0,false);
hitbox_data[STATES.air_heavy]	= new HitboxData(hitbox_spyke_air_heavy,8,90,10,3,-4,0,0,false,,,,,sfx_lightning_hit);
...
arr_state_functions[STATES.light] = function(){
	...
	xadd = approach(xadd,image_index < 1 ? ground_fric : slide_fric,0);
	yadd = 0;
...
arr_state_functions[STATES.dead] = function(){
	...
	if(state_changed){
		...
		//left
		var _inst = instance_create_depth((bbox_left+bbox_right)/2,(bbox_bottom+bbox_top)/2,depth,obj_spear_phy);
		_inst.xadd = xadd*4;
		_inst.yadd = min(yadd*4,-12);
		_inst.apply_phy()
